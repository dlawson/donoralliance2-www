...
function is_IE8 (){
	$browserInfo = php_browser_info();
	if(isset($browserInfo['browser']) && $browserInfo['browser']=='IE' && strpos( $browserInfo['browser_name_pattern'], 'MSIE 8' ) > 0)
		return true;
	return false;	
}
...
function is_lte_IE8 () {
	return (is_IE8() || is_lt_IE8());
}

function is_XP() {
	$browserInfo = php_browser_info();
	if(isset($browserInfo['platform']) && $browserInfo['platform']=='WinXP')
		return true;
	return false;
}
